...
async function main() {
  ...
  const SmartString = await hre.ethers.getContractFactory('SmartString');
  const smartString = await SmartString.deploy();
  ...
  await deployed('SmartString', hre.network.name, smartString.address);
}
